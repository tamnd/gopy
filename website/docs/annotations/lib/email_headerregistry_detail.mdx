...
a type-specific class depending on the header name.
...
looks up the registered class for that name.
...
`AddressHeader.parse` calls `email._parseaddr` to turn a raw string into
`(display_name, addr_spec)` tuples, then wraps each result in an `Address` object.
...
Not yet ported. Planned at `module/email/headerregistry`. The Go mirror will need a
`HeaderRegistry` map from string to a factory function, and concrete types for `Address`,
`Group`, and the date/MIME families.
