...
| 142-165 | `readmodule(module, path=None)` | Public entry point. Returns a `{name: Class}` dict (functions excluded). | not ported |
| 167-190 | `readmodule_ex(module, path=None)` | Public entry point. Returns a `{name: Class|Function}` dict (everything included). | not ported |
...
`Class.super` is a list that may contain either string names (when the base class lives in another module and has not been resolved yet) or actual `Class` objects (after `_create_tree` resolves them). `Class.methods` is a `{name: lineno}` dict populated by `_readmodule` as it walks `FunctionDef` nodes inside the class body.
