...
case-insensitive because RFC 5322 says header names are case-insensitive.
...
The stored values are raw strings. `header_fetch_parse` from the active `Policy` turns them
into structured objects when the new-style API is in use.
