...
| `test_source_encoding.py` | 547 | Lexer-side DONE. After spec 1718 P1-P9 (P9 ships the CJK codec subsystem under `codecs/cjkcodecs/`: multibytecodec runtime, cp932 / cp949 / euc_kr / euc_jp / shift_jis / johab / 2004 variants, plus the build-time `Tools/cjkcodecs_go` generator), `test_exec_valid_coding` now passes. Current run: 89 pass / 1 skip / 1 error out of 91. The remaining error is `test_issue2301`: cp932 decode succeeds, but `compile()` drops the parser's structured SyntaxError text on the `ErrParserNotImplemented` path; the same `v.text == None` gap reproduces for ASCII source, so the blocker lives in compile()/parser, not the codec subsystem. | spec 1718 P1-P9; df8bcf2 |
