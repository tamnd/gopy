...
| `test_source_encoding.py` | 547 | Lexer-side DONE. After spec 1718 P1-P5 the suite runs end-to-end (91 tests, 12 failures + 60 errors). Lexer/tokenizer no longer the blocker; remaining gaps are downstream subsystems: `UnicodeDecodeError` message template (`'ascii' codec can't decode byte 0xXX in position N: ordinal not in range(128)`), the lexer "Non-UTF-8 code starting with ... on line N" error formatting, and `compile()` / `__import__` raising `SyntaxError` on bad encoding instead of returning `None`. | spec 1718 P1-P5 |
