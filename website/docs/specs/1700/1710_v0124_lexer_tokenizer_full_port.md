...
| `test_source_encoding.py` | 547 | Lexer-side DONE. After spec 1718 P1-P5 plus the parser+vm SyntaxError propagation port (`runParse` now returns a typed `*parser/errors.SyntaxError` and `synthesizeException` builds the exception via the 2-arg `(msg, info)` form so `lineno` / `offset` / `text` / `filename` populate), 91/11/59 tests pass/fail/error. The lexer Non-UTF-8 message now carries "on line N" byte-for-byte with CPython. Remaining failures are downstream subsystems: `UnicodeDecodeError` message template (`'ascii' codec can't decode byte 0xXX in position N: ordinal not in range(128)`), `os.PathLike`, `_io` fileno, and `__import__` raising `SyntaxError` on bad encoding rather than returning `None`. | spec 1718 P1-P5; a3c4dbd, e603c6d |
