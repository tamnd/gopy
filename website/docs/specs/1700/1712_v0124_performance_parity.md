...
| P2.2 | Port `Python/optimizer_bytecodes.c` in full (1107 LOC, 114 abstract-interp cases). This is the abstract-interpreter case table that `optimize_uops` dispatches through. Lands as `optimizer/optimizer_bytecodes_gen.go`. Gate: every uop ID has a corresponding case body (no `unknown semantics` bail). Broken down into sub-buckets P2.2.A through P2.2.H below. | WIP | - |
| P2.3 | Port `Python/executor_cases.c.h` in full (7163 LOC, 271 stub uops). Replaces the deopt-pass-through stubs in `optimizer/uops_stubs_gen.go` with real executable bodies (Tier-2 stays in-process for now; the eventual `vm/eval_uops_gen.go` split lands after the bodies). Gate: every uop ID has a real executable body. Broken down into sub-buckets P2.3.A through P2.3.L below. | WIP | - |

**P2.2 sub-buckets (`Python/optimizer_bytecodes.c`, 114 abstract-interp cases).** Each row covers a contiguous group of `op(...)` blocks in CPython 3.14. The abstract interpreter is a sym-eval pass: every uop receives input symbols, produces output symbols, possibly emits constants the trace projector can fold, and possibly raises `unknown semantics` to bail out of optimization. Ship order mirrors `Python/optimizer_bytecodes.c` top to bottom because later cases reuse helper macros from earlier cases.

| Sub | Cases | Description | Status | Commit |
|-----|------:|-------------|--------|--------|
| P2.2.A | 9 | Fast locals + push-null + guards on TOS / NOS / type version (`_LOAD_FAST_CHECK`, `_LOAD_FAST`, `_LOAD_FAST_BORROW`, `_LOAD_FAST_AND_CLEAR`, `_STORE_FAST`, `_PUSH_NULL`, `_GUARD_TOS_INT`, `_GUARD_NOS_INT`, `_GUARD_TYPE_VERSION`). CPython: `Python/optimizer_bytecodes.c:77-143`. | TODO | - |
| P2.2.B | 11 | Arithmetic + binary ops abstract (`_BINARY_OP`, `_BINARY_OP_{ADD,SUBTRACT,MULTIPLY}_{INT,FLOAT}`, `_BINARY_OP_{ADD,INPLACE_ADD}_UNICODE`, `_GUARD_{TOS,NOS}_FLOAT`, `_GUARD_{TOS,NOS}_UNICODE`). CPython: `Python/optimizer_bytecodes.c:144-449`. | TODO | - |
| P2.2.C | 10 | Subscription + to-bool abstract (`_BINARY_OP_SUBSCR_*`, `_TO_BOOL`, `_TO_BOOL_BOOL`, `_TO_BOOL_INT`, `_TO_BOOL_LIST`, `_TO_BOOL_NONE`, `_TO_BOOL_STR`). CPython: `Python/optimizer_bytecodes.c:364-431`, `:450-498`. | TODO | - |
| P2.2.D | 18 | Attribute access abstract + guards (`_LOAD_ATTR`, `_LOAD_ATTR_INSTANCE_VALUE`, `_LOAD_ATTR_MODULE`, `_LOAD_ATTR_WITH_HINT`, `_LOAD_ATTR_SLOT`, `_LOAD_ATTR_CLASS`, `_LOAD_ATTR_METHOD_*`, `_STORE_ATTR_*`, `_CHECK_ATTR_*`, `_GUARD_DORV_*`, `_GUARD_KEYS_VERSION`, `_GUARD_GLOBALS_VERSION`). | TODO | - |
| P2.2.E | 15 | Calls + frame construction (`_CALL`, `_PY_FRAME_GENERAL`, `_CHECK_FUNCTION_*`, `_INIT_CALL_*`, `_PUSH_FRAME`, `_RESUME_CHECK`, `_TIER2_RESUME_CHECK`, `_SAVE_RETURN_OFFSET`, `_CHECK_STACK_SPACE`, `_CHECK_RECURSION_REMAINING`, `_CHECK_PEP523`). | TODO | - |
| P2.2.F | 12 | Iteration + generator + send (`_FOR_ITER_TIER_TWO`, `_ITER_*`, `_GET_ITER`, `_GET_YIELD_FROM_ITER`, `_GUARD_NOT_EXHAUSTED_*`, `_SEND_GEN_FRAME`, `_YIELD_VALUE`). | TODO | - |
| P2.2.G | 12 | Build / unpack / format (`_BUILD_LIST`, `_BUILD_MAP`, `_BUILD_SET`, `_BUILD_SLICE`, `_BUILD_STRING`, `_BUILD_TUPLE`, `_LIST_APPEND`, `_LIST_EXTEND`, `_SET_ADD`, `_SET_UPDATE`, `_FORMAT_SIMPLE`, `_FORMAT_WITH_SPEC`). | TODO | - |
| P2.2.H | 27 | Imports + exception handling + control flow tail (`_IMPORT_NAME`, `_IMPORT_FROM`, `_CHECK_EXC_MATCH`, `_CHECK_EG_MATCH`, `_POP_EXCEPT`, `_LOAD_GLOBAL_*`, `_LOAD_CONST_*`, `_LOAD_SMALL_INT*`, `_LOAD_SPECIAL`, `_LOAD_DEREF`, `_STORE_DEREF`, `_DELETE_*`, `_MAKE_FUNCTION`, `_SET_FUNCTION_ATTRIBUTE`, `_MAKE_CELL`, `_COPY_FREE_VARS`, `_CONVERT_VALUE`, `_RETURN_VALUE`, `_RETURN_GENERATOR`, `_END_FOR`, `_END_SEND`, `_FATAL_ERROR`, `_DEOPT`, `_ERROR_POP_N`, `_INSERT_NULL`, `_EXIT_INIT_CHECK`). | TODO | - |

**P2.3 sub-buckets (`Python/executor_cases.c.h`, 271 stub uops).** Each row covers one uop family; the stub method name uses gopy's CamelCase translation of the uop ID (e.g. `_LOAD_FAST_0` becomes `uopLoadFast0`). Ship order targets bench impact: the families that fire in nbody / spectral-norm / float / chaos / pyflate first, then the long tail. Cases column counts the existing stubs in `optimizer/uops_stubs_gen.go` that this bucket subsumes.

| Sub | Cases | Description | Status | Commit |
|-----|------:|-------------|--------|--------|
| P2.3.A | 8 | Fast-locals specialized arms: `_LOAD_FAST_0` through `_LOAD_FAST_7`, `_LOAD_FAST_BORROW_0` through `_LOAD_FAST_BORROW_7`, `_STORE_FAST_0` through `_STORE_FAST_7`, `_LOAD_FAST_CHECK`, `_LOAD_FAST_AND_CLEAR`. Hot in every benchmark; trivial wrappers around the existing `uopLoadFast` / `uopStoreFast` bodies with the oparg pinned. | TODO | - |
| P2.3.B | 7 | Constant loads: `_LOAD_CONST_IMMORTAL`, `_LOAD_CONST_MORTAL`, `_LOAD_CONST_INLINE`, `_LOAD_CONST_INLINE_BORROW`, `_LOAD_COMMON_CONSTANT`, `_LOAD_SMALL_INT` / `_LOAD_SMALL_INT_{0,1,2,3}`, `_POP_TOP_LOAD_CONST_INLINE`, `_POP_TOP_LOAD_CONST_INLINE_BORROW`, `_POP_TWO_LOAD_CONST_INLINE_BORROW`. CPython: `Python/executor_cases.c.h` LOAD_CONST family. | TODO | - |
| P2.3.C | 18 | Arithmetic + unary + binary specialized: `_BINARY_OP`, `_BINARY_OP_ADD_INT`, `_BINARY_OP_SUBTRACT_INT`, `_BINARY_OP_MULTIPLY_INT`, `_BINARY_OP_*_FLOAT` (3), `_BINARY_OP_ADD_UNICODE`, `_BINARY_OP_INPLACE_ADD_UNICODE`, `_BINARY_OP_EXTEND`, `_GUARD_BINARY_OP_EXTEND`, `_UNARY_INVERT`, `_UNARY_NEGATIVE`, `_UNARY_NOT`. Hot path for nbody, spectral_norm, float, chaos. | TODO | - |
| P2.3.D | 15 | Subscript + slice: `_BINARY_OP_SUBSCR_DICT`, `_BINARY_OP_SUBSCR_LIST_INT`, `_BINARY_OP_SUBSCR_LIST_SLICE`, `_BINARY_OP_SUBSCR_STR_INT`, `_BINARY_OP_SUBSCR_TUPLE_INT`, `_BINARY_OP_SUBSCR_CHECK_FUNC`, `_BINARY_OP_SUBSCR_INIT_CALL`, `_BINARY_SLICE`, `_STORE_SUBSCR`, `_STORE_SUBSCR_DICT`, `_STORE_SUBSCR_LIST_INT`, `_STORE_SLICE`, `_DELETE_SUBSCR`. | TODO | - |
| P2.3.E | 22 | Guard primitives that gate the rest: `_GUARD_TYPE_VERSION`, `_GUARD_TYPE_VERSION_AND_LOCK`, `_GUARD_TOS_INT`, `_GUARD_NOS_INT`, `_GUARD_TOS_FLOAT`, `_GUARD_NOS_FLOAT`, `_GUARD_TOS_UNICODE`, `_GUARD_NOS_UNICODE`, `_GUARD_TOS_LIST`, `_GUARD_NOS_LIST`, `_GUARD_TOS_TUPLE`, `_GUARD_NOS_TUPLE`, `_GUARD_TOS_DICT`, `_GUARD_NOS_DICT`, `_GUARD_TOS_ANY_SET`, `_GUARD_TOS_SLICE`, `_GUARD_NOS_NULL`, `_GUARD_KEYS_VERSION`, `_GUARD_GLOBALS_VERSION`, `_GUARD_DORV_NO_DICT`, `_GUARD_DORV_VALUES_INST_ATTR_FROM_DICT`, `_GUARD_NOT_EXHAUSTED_{LIST,TUPLE,RANGE}`, plus `_CHECK_*` guards. | TODO | - |
| P2.3.F | 33 | Attribute load + store + check: `_LOAD_ATTR`, `_LOAD_ATTR_INSTANCE_VALUE` (covered hand-port but stub still present for variants), `_LOAD_ATTR_MODULE`, `_LOAD_ATTR_WITH_HINT`, `_LOAD_ATTR_SLOT`, `_LOAD_ATTR_CLASS`, `_LOAD_ATTR_METHOD_*` (5 variants), `_LOAD_ATTR_NONDESCRIPTOR_*`, `_LOAD_ATTR_PROPERTY_FRAME`, `_STORE_ATTR`, `_STORE_ATTR_INSTANCE_VALUE`, `_STORE_ATTR_SLOT`, `_STORE_ATTR_WITH_HINT`, `_DELETE_ATTR`, `_CHECK_ATTR_CLASS`, `_CHECK_ATTR_METHOD_LAZY_DICT`, `_CHECK_AND_ALLOCATE_OBJECT`, `_CHECK_MANAGED_OBJECT_HAS_VALUES`, `_LOAD_SUPER_ATTR_ATTR`, `_LOAD_SUPER_ATTR_METHOD`. | TODO | - |
| P2.3.G | 33 | Calls + frame: `_CALL_BUILTIN_CLASS`, `_CALL_BUILTIN_FAST{,_WITH_KEYWORDS}`, `_CALL_BUILTIN_O`, `_CALL_INTRINSIC_{1,2}`, `_CALL_ISINSTANCE`, `_CALL_LEN`, `_CALL_LIST_APPEND`, `_CALL_METHOD_DESCRIPTOR_*` (4), `_CALL_NON_PY_GENERAL`, `_CALL_KW_NON_PY`, `_CALL_STR_1`, `_CALL_TUPLE_1`, `_CALL_TYPE_1`, `_PY_FRAME_GENERAL`, `_PY_FRAME_KW`, `_PUSH_FRAME`, `_PUSH_NULL_CONDITIONAL`, `_INIT_CALL_BOUND_METHOD_EXACT_ARGS`, `_INIT_CALL_PY_EXACT_ARGS{,_0,_1,_2,_3,_4}`, `_CHECK_FUNCTION{,_EXACT_ARGS,_VERSION,_VERSION_INLINE,_VERSION_KW}`, `_CHECK_IS_NOT_PY_CALLABLE{,_KW}`, `_CHECK_METHOD_VERSION{,_KW}`, `_CHECK_CALL_BOUND_METHOD_EXACT_ARGS`, `_CHECK_PEP523`, `_CHECK_PERIODIC{,_IF_NOT_YIELD_FROM}`, `_CHECK_RECURSION_REMAINING`, `_CHECK_STACK_SPACE{,_OPERAND}`, `_CREATE_INIT_FRAME`, `_EXIT_INIT_CHECK`, `_EXPAND_METHOD{,_KW}`, `_MAYBE_EXPAND_METHOD{,_KW}`, `_MAKE_CALLARGS_A_TUPLE`, `_RESUME_CHECK`, `_TIER2_RESUME_CHECK`, `_SAVE_RETURN_OFFSET`. | TODO | - |
| P2.3.H | 12 | Iteration + generator: `_FOR_ITER_GEN_FRAME`, `_FOR_ITER_TIER_TWO`, `_GET_AITER`, `_GET_ANEXT`, `_GET_AWAITABLE`, `_GET_ITER`, `_GET_LEN`, `_GET_YIELD_FROM_ITER`, `_ITER_CHECK_{LIST,RANGE,TUPLE}`, `_ITER_NEXT_{LIST_TIER_TWO,RANGE,TUPLE}`, `_END_FOR`, `_END_SEND`, `_SEND_GEN_FRAME`, `_YIELD_VALUE`. | TODO | - |
| P2.3.I | 18 | Build + unpack + format: `_BUILD_INTERPOLATION`, `_BUILD_LIST`, `_BUILD_MAP`, `_BUILD_SET`, `_BUILD_SLICE`, `_BUILD_STRING`, `_BUILD_TEMPLATE`, `_BUILD_TUPLE`, `_LIST_APPEND`, `_LIST_EXTEND`, `_SET_ADD`, `_SET_UPDATE`, `_DICT_MERGE`, `_DICT_UPDATE`, `_MAP_ADD`, `_UNPACK_EX`, `_UNPACK_SEQUENCE`, `_UNPACK_SEQUENCE_LIST`, `_UNPACK_SEQUENCE_TUPLE`, `_UNPACK_SEQUENCE_TWO_TUPLE`, `_FORMAT_SIMPLE`, `_FORMAT_WITH_SPEC`, `_CONVERT_VALUE`. | TODO | - |
| P2.3.J | 21 | Comparison + boolean + match: `_COMPARE_OP`, `_COMPARE_OP_FLOAT`, `_COMPARE_OP_INT`, `_COMPARE_OP_STR`, `_CONTAINS_OP`, `_CONTAINS_OP_DICT`, `_CONTAINS_OP_SET`, `_IS_OP`, `_IS_NONE`, `_TO_BOOL`, `_TO_BOOL_BOOL`, `_TO_BOOL_INT`, `_TO_BOOL_LIST`, `_TO_BOOL_NONE`, `_TO_BOOL_STR`, `_GUARD_IS_FALSE_POP`, `_GUARD_IS_TRUE_POP`, `_GUARD_IS_NONE_POP`, `_GUARD_IS_NOT_NONE_POP`, `_REPLACE_WITH_TRUE`, `_MATCH_CLASS`, `_MATCH_KEYS`, `_MATCH_MAPPING`, `_MATCH_SEQUENCE`. | TODO | - |
| P2.3.K | 17 | Globals + names + cells: `_LOAD_GLOBAL`, `_LOAD_GLOBAL_BUILTINS`, `_LOAD_GLOBAL_MODULE`, `_LOAD_LOCALS`, `_LOAD_NAME`, `_LOAD_BUILD_CLASS`, `_LOAD_DEREF`, `_LOAD_FROM_DICT_OR_DEREF`, `_LOAD_SPECIAL`, `_STORE_GLOBAL`, `_STORE_NAME`, `_STORE_DEREF`, `_DELETE_NAME`, `_DELETE_GLOBAL`, `_DELETE_FAST`, `_DELETE_DEREF`, `_MAKE_CELL`, `_COPY_FREE_VARS`. | TODO | - |
| P2.3.L | 16 | Exception + tail: `_CHECK_EG_MATCH`, `_CHECK_EXC_MATCH`, `_POP_EXCEPT`, `_PUSH_EXC_INFO`, `_WITH_EXCEPT_START`, `_MAKE_FUNCTION`, `_SET_FUNCTION_ATTRIBUTE`, `_RETURN_GENERATOR`, `_RETURN_VALUE`, `_IMPORT_FROM`, `_IMPORT_NAME`, `_GUARD_CALLABLE_{LEN,STR_1,TUPLE_1,TYPE_1}`, `_DEOPT`, `_FATAL_ERROR`, `_ERROR_POP_N`, `_INSERT_NULL`, `_SETUP_ANNOTATIONS`. | TODO | - |
