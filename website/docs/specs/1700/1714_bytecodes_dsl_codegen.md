...
| Step | Status | Commit |
|------|--------|--------|
| `tools/cases_generator/gowriter.py` lands | TODO | - |
| `tools/cases_generator/go_generators_common.py` lands with the binding table above | TODO | - |
| 30-snippet golden-file corpus under `tools/cases_generator/testdata/snippets/` | TODO | - |
| `go test ./tools/cases_generator -run TestSnippetParity` (via a small Go test harness that shells out to the Python) green | TODO | - |
...
| Step | Status | Commit |
|------|--------|--------|
| `gopy_family_generator.py` (or fold into Phase 2's metadata emitter) | TODO | - |
| `specialize/family_gen.go` lands | TODO | - |
| `specialize/quicken.go` + `specialize/deopt.go` consume generated tables | TODO | - |
| Parity test green; old literals deleted | TODO | - |
...
| Step | Status | Commit |
|------|--------|--------|
| `gopy_tier1_generator.py` lands | TODO | - |
| `vm/eval_dispatch_gen.go` covers every unspecialized opcode | TODO | - |
| `vm/eval_simple.go` shrinks to evalLoop scaffolding only (frame setup, exit handling) | TODO | - |
| `go test ./vm` green | TODO | - |
...
- [ ] Phase 1.3 — 30-snippet golden corpus under `tools/cases_generator/testdata/snippets/`
...
- [ ] Phase 5.1 — `gopy_tier1_generator.py` emits `vm/eval_dispatch_gen.go` for unspecialized opcodes
