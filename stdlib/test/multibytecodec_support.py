...
# True when lone surrogates survive round-trip through Python strings.
# CPython 3 preserves them; Go-based runtimes (gopy) replace them with U+FFFD.
_SURROGATE_OK = ('\udeee' != '�')
...
class TestBase:
    ...
    def _skip_if_iso10646_no_surrogate(self):
        if self.has_iso10646 and not _SURROGATE_OK:
            self.skipTest("lone surrogates not supported on this platform")
    ...
    def test_callback_returns_bytes(self):
        self._skip_if_iso10646_no_surrogate()
        def myreplace(exc):
...
class TestBase_Mapping(unittest.TestCase):
    ...
    def setUp(self):
        ...
        except (OSError, ImportError, HTTPException):
