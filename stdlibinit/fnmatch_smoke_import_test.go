...
func TestImportFnmatchBareSmoke(t *testing.T) {
	installStdlibFinder(t)
	t.Cleanup(func() {
		imp.RemoveModule("fnmatch")
	})
	...
	ts := state.NewThread()
	src := "import fnmatch\n"
	if _, err := pythonrun.RunString(ts, src, "<fnmatch-import>", parser.ModeFile, g, nil); err != nil {
		t.Fatalf("import fnmatch: %v", err)
	}
}
